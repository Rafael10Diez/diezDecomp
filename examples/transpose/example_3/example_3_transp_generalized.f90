program main
  ...
  real(rp) , allocatable, target    ::  p_out_ref(:,:,:), p_in_buf(:), p_out_buf(:)
  ...
  block
    integer :: n0_in, n1_in, n2_in, n0_out, n1_out, n2_out,min_size
    ...
    min_size = max(n0_in *n1_in *n2_in, n0_out*n1_out*n2_out)
    allocate(p_in_buf (0:(min_size-1)))
    allocate(p_out_buf(0:(min_size-1)))
    p_out_ref  = -1
    p_in_buf   = -1
    p_out_buf  = -1
    p_in (0:n0_in -1, 0:n1_in -1, 0:n2_in -1) => p_in_buf (0:(n0_in *n1_in *n2_in -1))
    p_out(0:n0_out-1, 0:n1_out-1, 0:n2_out-1) => p_out_buf(0:(n0_out*n1_out*n2_out-1))
    p_out = -1
  end block
  ...
  !$acc wait
  !$acc enter data copyin(p_in_buf,p_out_buf)
  !$acc wait

  ! execute transpose
  call diezdecomp_transp_execute_generic_nobuf(tr, p_in, p_out)
  !$acc wait
  !$acc exit data copyout(p_in_buf,p_out_buf)
  !$acc wait
  ...
  block
    ...
    j1 = j0 + ny_out - 1
    k1 = k0 + nz_out - 1
    error_max = maxval(abs(p_out(i0:i1,j0:j1,k0:k1) - p_out_ref(i0:i1,j0:j1,k0:k1)))
  end block
  ...
  if (irank == 0) then
    write(6,'(A68,E12.4,A1,A36)') 'diezDecomp: example_3_transp_generalized.f90: SUCCESS!!! error_max: ', error_max,&
                                  ' ', fname_36 ; flush(6)
  end if
